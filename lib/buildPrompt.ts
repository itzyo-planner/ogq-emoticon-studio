...
const automaticPromptAppendixBase = `
...
const automaticPromptAppendixSuffix = `
...
const buildAutomaticPromptAppendix = (noText: boolean, noOutline: boolean): string => {
  let appendix = automaticPromptAppendixBase;
  if (!noText) {
    appendix += textPromptAppendix;
  }
  appendix += automaticPromptAppendixSuffix;
  if (!noOutline) {
    appendix += outlineAppendix;
  }
  if (!noText) {
...
export const buildPrompt = (
  ...
  const noOutline = options?.noOutline ?? false;
  ...
  const stickerLabel = noOutline
    ? '**FLAT 2D STICKER** (character only, no white border, no frame)'
    : '**FLAT 2D STICKER** (character only, no white border, no frame)';

  const outlineRules = noOutline
    ? ''
  ...
  const negativePromptParts = [
    ...
    'white border, die-cut border, white frame, white stroke, border frame, sticker border',
  ];

  if (noText) {
    negativePromptParts.push('text, typography, handwriting, calligraphy, korean text, written text');
  }

  if (noOutline) {
    negativePromptParts.push('black outline, line art, contour line, border line, stroke, linework');
  }

  const automaticPromptAppendix = buildAutomaticPromptAppendix(noText, noOutline);

  return `
    Design a ${stickerLabel} asset.

    **SUBJECT**: ${characterDesc}
    **ACTION/EMOTION**: ${scenarioPrompt}
    **ART STYLE**: ${noOutline ? `FLAT 2D STICKER (라인아트 없이 플랫 컬러 스타일) — ${style}` : style}
